...
class OptimizationPass : public StateMentPass{
        ...
        vector_(vector)
    {
        ...
        float finf_vec[vector_];
        for( int i = 0 ; i < vector_ ; i++ ) {
            finf_vec[i] = INFINITY;
	    }

        finf_vec_const_ = new Const( finf_vec , vector_ );
            ...
            if(reduction_info.order_type_ == IncContinue) {
                ...
                for( int reduce_i = 0 ; reduce_i < reduce_num ; reduce_i++ ) {
                    ...
                    if(v2_state_new_type.get_data_type() == DOUBLE) { 
                        ...
                        reduce_state_vec.push_back(LetStat::make( shuffle_simd, Shuffle::make( shuffle_res , finf_vec_const_, shuffle_index_const_vec[reduce_i] )));
                    } else {
                        LOG(FATAL) << "Unsupported";
                    }
                ...
                if(v2_state_new_type_basic_data_type == DOUBLE) { 
                    ...
                    // TODO: inf
                    reduce_state_vec.push_back(LetStat::make( shuffle_res, Shuffle::make( shuffle_res , finf_vec_const_, compress_const )));
                } else {
                ...
                for( int reduce_i = 0 ; reduce_i < reduce_num ; reduce_i++ ) {
                  ...
                  //  LOG(INFO);
                    ...
                    if (shuffle_simd->get_type().get_data_type() == FLOAT) {
                        zero_vec_const_ = finf_vec_const_;
                    } else {
            ...
            if(reduction_info.order_type_ == IncContinue) {
                ...
                for( int reduce_i = 0 ; reduce_i < reduce_num ; reduce_i++ ) {
                  ...
                  //  LOG(INFO);
                    ...
                    Varience * select_var = new Varience( v2_state_new->get_type());
                    if(v2_state_new->get_type().get_data_type() == DOUBLE) { 
                        reduce_state_vec.push_back( LetStat::make(select_var, Select::make(  dzero_vec_const_,shuffle_simd, mask_var ) ) );
                    } else {
                        ...
                        if(gather_state!=NULL ) {
                            ...
                            if(min_state!=NULL ) {
                                ...
                                if(reduce_var->get_type() == __double_v) {
                                    ...
                                    outer_init_state_vec.push_back(LetStat::make(reduce_var,finf_vec_const_ )) ;
                                } else {
                                    LOG(FATAL) << "Unsupported";
                                }
