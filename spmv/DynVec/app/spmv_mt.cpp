...
void spmv_dynvec(FuncType func, double* y_array, int* row_ptr_all, int* column_ptr, double* x_array, double* data_ptr, int data_num) {
    printf("vector_nums = %d, y_array = %p, row_ptr_all = %p, column_ptr = %p, x_array = %p, data_ptr = %p, data_num = %d\n", vector_nums, y_array, row_ptr_all, column_ptr, x_array, data_ptr, data_num);
    func( y_array,row_ptr_all, column_ptr, x_array,data_ptr );
    for( int i = data_num / vector_nums * vector_nums ; i < data_num ; i++ ) {
	printf("y_array[%d] = %lf += x_array[%d] = %lf * data_ptr[%d] = %lf", row_ptr_all[ i ], y_array[ row_ptr_all[ i ] ], column_ptr[i], x_array[column_ptr[i]], i, data_ptr[ i ]);
...
int main( int argc , char const * argv[] ) {
    ...
    for (int i = 0; i < thread_num; ++i)
    {
            ...
            //     printf("%d: %d\n", p, row_ptr[p]);
            // }
            const int row_num = min(sparseMatrixPtr->row_num*(i+1)/thread_num, sparseMatrixPtr->row_num) \
                - sparseMatrixPtr->row_num * i / thread_num;
            ...
            for(int row_i = 0 ; row_i < row_num; row_i++) {
                ...
                for( int j = begin ; j < end ; j++ ) {
                    row_ptr_all[j] = row_i+sparseMatrixPtr->row_num*i/thread_num;
                }
            }
            std::string spmv_str = 
            "input: int * row_ptr,   \
