...
inline uint64_t  Armv8_CNTPCT_EL0_read()
{
	uint64_t val = 0;
...
inline uint64_t  Armv8_CNTFRQ_EL0_read()
{
	uint64_t val = 0;
...
extern struct timespec timer_begin, timer_end;
extern clock_t begin_t, end_t;
extern volatile uint64_t begin_pct, end_pct;
extern volatile uint64_t begin_frq, end_frq;
